    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (Input.Role == SDRoles.Role_User_Comp)
                {
                    user.CompanyId = Input.CompanyId;
                }
                if (Input.Role == SDRoles.Role_ShopAdmin)
                {
                    user.ShopId = Input.ShopId;
                }
                if (Input.Role == SDRoles.Role_ShopUser || Input.Role == SDRoles.Role_ShopEmployee || Input.Role == SDRoles.Role_Saleman)
                {
                    var claimsIdentity = (ClaimsIdentity)User.Identity;
                    var claim = claimsIdentity.FindFirst(ClaimTypes.NameIdentifier);
                    ApplicationUser applicationUser = _unitOfWork.ApplicationUser.GetFirstOrDefault(x => x.Id == claim.Value);
                    user.ShopId = applicationUser.ShopId;
                }
                ...
                if (result.Succeeded)
                {
                    ...
                    if (Input.Role == null)
                    {
                        await _userManager.AddToRoleAsync(user, SDRoles.Role_User_Indi);
                    }
                    else
                    {
