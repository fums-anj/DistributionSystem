...
namespace BHWeb.Areas.SaleManagement.Controllers
{
    ...
    public class ShopOrderController : BaseController
    {
        ...
        public ShopOrderController(IUnitOfWork unitOfWork)
        {
        ...
        {
            SaleOrderVM saleOrderVM = new();
            saleOrderVM.UserList = new SelectList(_unitOfWork.ApplicationUser.GetAll(x => x.ShopId == applicationUser.ShopId), "Id", "FullName");
            var claimsIdentity = (ClaimsIdentity)User.Identity;
            var claim = claimsIdentity.FindFirst(ClaimTypes.NameIdentifier);
            saleOrderVM.SaleOrderList = _unitOfWork.SaleOrder.GetAll(u => u.CreatedBy == claim.Value && u.ShopId == applicationUser.ShopId && u.CreatedDate >= DateTime.Now.Date, includeProperties: "ApplicationUser,Customer").OrderByDescending(o => o.Id);
            return View(saleOrderVM);
        }
        [HttpPost]
        [AutoValidateAntiforgeryToken]
        public IActionResult Index(SaleOrderVM saleOrderVM)
        {
            saleOrderVM.UserList = new SelectList(_unitOfWork.ApplicationUser.GetAll(x => x.ShopId == applicationUser.ShopId), "Id", "FullName");
            var claimsIdentity = (ClaimsIdentity)User.Identity;
            var claim = claimsIdentity.FindFirst(ClaimTypes.NameIdentifier);
            saleOrderVM.SaleOrderList = _unitOfWork.SaleOrder.GetAll(u => u.ShopId == applicationUser.ShopId, includeProperties: "ApplicationUser,Customer").OrderByDescending(o => o.Id);
            saleOrderVM.SaleOrderList = _unitOfWork.SaleOrder.FilterSaleOrder(saleOrderVM);
            return View(saleOrderVM);
        }
